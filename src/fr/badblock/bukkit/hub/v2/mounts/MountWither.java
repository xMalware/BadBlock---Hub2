...
public class MountWither extends EntityWither{
	...
	public MountWither(World world) {
		super(world);
		if(FIELD_JUMP == null) {
			try {
	...
	public static Wither spawnEntity(Location location) {
		World world = (World) ((CraftWorld) location.getWorld()).getHandle();
		MountWither wither = new MountWither(world);
		wither.setLocation(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
		((CraftLivingEntity) wither.getBukkitEntity()).setRemoveWhenFarAway(false);
		world.addEntity(wither, SpawnReason.CUSTOM);
		wither.setCustomName("");
		wither.setCustomNameVisible(false);
		return (CraftWither) wither.getBukkitEntity();
	}
}
