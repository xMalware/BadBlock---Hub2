...
public class MountCaveSpider extends EntityCaveSpider{

	protected Field FIELD_JUMP = null;
	
	public MountCaveSpider(World world) {
		super(world);
		if(FIELD_JUMP == null) {
			try {
	...
	public static CaveSpider spawnEntity(Location location) {
		World world = (World) ((CraftWorld) location.getWorld()).getHandle();
		MountCaveSpider caveSpider = new MountCaveSpider(world);
		caveSpider.setLocation(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
		((CraftLivingEntity) caveSpider.getBukkitEntity()).setRemoveWhenFarAway(false);
		world.addEntity(caveSpider, SpawnReason.CUSTOM);
		caveSpider.setCustomName("");
		caveSpider.setCustomNameVisible(false);
		return (CraftCaveSpider) caveSpider.getBukkitEntity();
	}
}
