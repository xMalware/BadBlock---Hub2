...
public class MountVillager extends EntityVillager{

	protected Field FIELD_JUMP = null;
	
	public MountVillager(World world) {
		super(world);
		if(FIELD_JUMP == null) {
			try {
	...
	public static Villager spawnEntity(Location location, int i) {
		World world = (World) ((CraftWorld) location.getWorld()).getHandle();
		MountVillager villager = new MountVillager(world);
		villager.setLocation(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
		((CraftLivingEntity) villager.getBukkitEntity()).setRemoveWhenFarAway(false);
		world.addEntity(villager, SpawnReason.CUSTOM);
		villager.setCustomName("");
		villager.setCustomNameVisible(false);
		villager.setProfession(i);
