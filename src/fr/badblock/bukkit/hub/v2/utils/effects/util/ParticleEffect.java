...
import fr.badblock.bukkit.hub.v2.utils.effects.util.ReflectionUtils.PackageType;
...
public enum ParticleEffect {
	 ...
	 * </ul>
	 */
	CRIT_MAGIC("magicCrit", 10, -1), DAMAGE_INDICATOR("damageIndicator", 44, 9), DRAGON_BREATH("dragonbreath", 42, 9),
	/**
	 ...
	 * </ul>
	 */
	ENCHANTMENT_TABLE("enchantmenttable", 25, -1), END_ROD("endRod", 43, 9),
	/**
	 ...
	 * </ul>
	 */
	SUSPENDED_DEPTH("depthSuspend", 8, -1), SWEEP_ATTACK("sweepAttack", 45, 9),
	/**
		...
		public static void initialize() throws VersionIncompatibleException {
			...
			try {
				...
				try {
					...
					packetConstructor = ReflectionUtils.getConstructor(packetClass);
					getHandle = ReflectionUtils.getMethod("CraftPlayer", PackageType.CRAFTBUKKIT_ENTITY, "getHandle");
					playerConnection = Class.forName("net.minecraft.entity.player.EntityPlayerMP")
							.getDeclaredField("field_71135_a");
					sendPacket = playerConnection.getType().getDeclaredMethod("func_147359_a",
	...
	public void display(float offsetX, float offsetY, float offsetZ, float speed, int amount, Location center,
		...
		if (!isSupported()) {
			throw new ParticleVersionException("The " + this
					+ " particle effect is not supported by your server version " + ParticlePacket.getVersion());
		}
		if (requiresData) {
	...
	public ParticleData getData(Material material, Byte blockData) {
		...
		if (this == ParticleEffect.BLOCK_CRACK || this == ParticleEffect.ITEM_CRACK
				|| this == ParticleEffect.BLOCK_DUST) {
